...
import { Card, SelectField, TextField, CheckField } from '../components/Field.jsx';
...
export function DataTab({ settings, update, reload, notify }) {
  const [syncStatus, setSyncStatus] = useState(null);
  const [syncing, setSyncing] = useState(false);
  const [confirm, setConfirm] = useState(null); // null | 'reset' | { type:'importAll', bundle }
  const settingsFileRef = useRef(null);
  const bundleFileRef = useRef(null);

  useEffect(() => {
    const read = () => chrome.storage.local.get('githubSyncStatus')
      .then(({ githubSyncStatus }) => setSyncStatus(githubSyncStatus || null));
    read();
  ...
  const importSettings = async (e) => {
    ...
    try {
      ...
      notify({ severity: 'error', message: `导入失败：${err.message}` });
    }
  };
