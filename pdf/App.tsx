...
import { extractTextFromFile } from './lib/pdfExtract.ts';
import type { Toast } from '../types/models.ts';

/** 逐段翻译的进度。finished 用来在完成后短暂显示"翻译完成"再淡出 */
...
export function App() {
  ...
  useEffect(() => {
    chrome.storage.sync.get({ engine: 'google' }).then((s) => setEngine(s.engine as string));
  }, []);
  ...
  const handleFile = useCallback(async (file: File | undefined) => {
    ...
    try {
      ...
      setToast({ severity: 'error', message: `文件读取失败：${(err as Error).message}` });
    }
  }, []);
