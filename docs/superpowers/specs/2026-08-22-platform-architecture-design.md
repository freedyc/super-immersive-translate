...
| # | 内容 | 状态 |
|---|------|------|
| 一 | 字幕平台化（9 平台 + 通用 cue 兜底 + 总开关） | ✅ 已完成并推送 |
| 七 | tokens 加语法角色（role） | ✅ 已完成 |
| 八 | React 迁移 | ✅ 六个页面全部完成，并额外迁到 TypeScript |
| 九 | 「翻译实验室」 | ✅ 以 sandbox 文字页的「语法分析」形态落地，未做成独立页面 |
| 四 | 共享 UI 反馈层 | ⚠️ 部分：独立页面统一用了 MUI Snackbar；内容脚本侧的手写 CSS 后端尚未做 |
| 二 | 文档渲染子系统（PDF 保留版式 + EPUB） | ⬜ 未开始（最大的一块） |
| 三 | 图片/漫画在页翻译 | ⬜ 未开始 |
| 五 | i18n 就绪约定（t() 查找） | ⬜ 未开始 |
| 六 | 术语表 | ⬜ 未开始 |

**实施中发现的、与本文档原有描述不符之处（已在正文更正）：**

1. **sandbox 的图片 OCR 不是 tesseract.js**，而是把图片 base64 传给第三方
   `api.ocr.space`（用的还是它的公开演示 key）。本文档第三节原先据此认为
   「OCR 逻辑已在 sandbox 里、抽出来即可复用」，这个前提不成立——真要做本地 OCR，
   `utils/ocr.js` 是从零开始写，不是抽取。这直接影响第三节的工作量估计。
2. **sandbox 的文档标签页原本是假功能**：跑假进度条、不读用户上传的文件、
   只翻译一句写死的示例文本。迁移时已改为复用 `pdf/lib/pdfExtract.ts` 真实实现。
3. `pdfExtract` 的解压降级分支传了非法的 `DecompressionStream('raw')`，一直抛异常、
   从未生效。加类型时被 tsc 揪出，已修为 `'deflate-raw'`。
...
- 点击后跑 OCR，识别出文字块的位置和内容。
- 把识别出的文字批量走现有 `Translator` 翻译，按每个文字块的坐标生成绝对定位的译文覆盖层叠加在原图对应位置上。

**OCR 方案需要先做决策，这是本节的主要未定项。** 本文档最初写着"复用 sandbox 已有的
tesseract.js OCR"，实施 React 迁移时发现这个前提是错的：sandbox 的图片翻译走的是第三方
`api.ocr.space`（公开演示 key），`tesseract.js` 虽然还在依赖里但那条路径并没有在用。
所以两个方向都得从零开始：

- **本地 tesseract.js**：隐私上干净（图片不出设备），但要下载几十 MB 的语言包，
  首次识别慢，且内容脚本里跑 WASM 会影响宿主页性能——需要放到 Web Worker 或 offscreen document。
- **继续用云端 OCR**：快、无体积负担，但把用户浏览的任意图片传给第三方，
  用在内容脚本里（面向所有网页、可能含私密截图）比用在 sandbox 里严重得多，
  必须至少做成显式开关 + 明确告知。

倾向本地方案，但需要先验证 WASM 在内容脚本环境下的实际开销再定。这块没定之前不要开工。
