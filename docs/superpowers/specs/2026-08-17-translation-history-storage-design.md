...
- `syncNow()`：`pullRemoteHistory()` → `mergeHistories(local, remote)` → 按 `historyMaxItems` 裁出本地要存的子集写 `chrome.storage.local`，同时把**未裁剪的合并全量**通过 `pushRemoteHistory()` 写回远端 → 更新 `chrome.storage.local` 里的 `githubSyncStatus: { lastSyncAt, lastError }`（本地专用状态，不参与同步）（**注意（分支整体审查后修正）**：为避免 `pullRemoteHistory()` 网络请求期间本地新写入被旧快照覆盖，实现里会在 `pullRemoteHistory()` 前后各读一次本地 `translationHistory`，两次读取都各自为缺 `id` 的旧记录回填 `id`；旧记录的 `id` 用 `crypto.subtle.digest('SHA-256', ...)` 对 `text|translation|engine|timestamp` 算出的确定性哈希生成（加 `legacy-` 前缀），同一条记录内容不变则无论回填多少次、在哪个执行上下文回填，结果都完全一样，因此两次独立读取即使各自生成新对象也不会得到不同 id，不需要"回填后立刻写回本地"这类额外步骤来防止 `mergeHistories` 误判重复）
