...
import { POS_BADGE_CLASS } from '../lib/mastery.ts';
import type { Token } from '../../types/models.ts';

export function TaggedSentence({
  sentence, tokens, className = '',
}: {
  sentence: string;
  tokens?: Token[];
  className?: string;
    ...
    <span className={`inline-flex flex-wrap gap-1 items-center ${className}`}>
      {tokens
        ...
        .map((tok, i) => (
          ...
          >
            <span className={`badge ${(tok.pos && POS_BADGE_CLASS[tok.pos]) || 'badge-neutral'} badge-sm align-middle`}>
              {tok.text}
            </span>
          </Tooltip>
        ))}
