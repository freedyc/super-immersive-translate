...
import { TaggedSentence } from '../components/TaggedSentence.jsx';
import { shuffled, latestContext, ROLE_ORDER } from '../lib/mastery.js';

const MODES = ['recall', 'recognition'];
...
// 只收"学过且到期"的 (词, 方向) 组合
function buildDueQueue(wordbook, now = new Date()) {
  const queue = [];
  wordbook.forEach((w) => {
...
// 只收"从没学过"的 (词, 方向) 组合
function buildNewQueue(wordbook) {
  const queue = [];
  wordbook.forEach((w) => {
    const translated = hasTranslation(w);
    MODES.forEach((mode) => {
...
function RecallQuestion({ word, onGrade, onRevealChange }) {
  const [answer, setAnswer] = useState('');
  const [state, setState] = useState(null); // null | 'correct' | 'wrong'
      ...
      <div className="card-body gap-4">
        ...
        {state === 'correct' && (
          <div className="flex justify-center gap-2">
            {[['hard', '困难'], ['good', '记得'], ['easy', '简单']].map(([grade, label]) => (
...
function RecognitionQuestion({ word, pool, onGrade }) {
  ...
  const choose = (i) => {
    ...
    const grade = !correct ? 'again' : elapsed < 2000 ? 'easy' : elapsed < 5000 ? 'good' : 'hard';
...
export function ReviewView({ wordbook, loaded, updateWord }) {
  const [queue, setQueue] = useState(null); // null = 还没初始化
  ...
  const recordResult = useCallback(async (grade) => {
    ...
    <div className="max-w-4xl mx-auto mt-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          ...
          style={{ '--value': done ? 100 : percent, '--size': '3.25rem', '--thickness': '4px' }}
          role="progressbar"
        >
