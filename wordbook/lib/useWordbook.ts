...
/** 就地修改一条单词记录的回调（拿到的是副本，改它就行） */
export type WordMutator = (word: WordEntry) => void;
...
export function useWordbook() {
  ...
  useEffect(() => {
    ...
    const onChanged = (
      ...
      if (area === 'local' && changes.wordbook) {
        setWordbook((changes.wordbook.newValue as WordEntry[]) || []);
      }
    };
  ...
  const persist = useCallback(async (next: WordEntry[]) => {
    setWordbook(next);
  ...
  const updateWord = useCallback(async (text: string, mutate: WordMutator) => {
